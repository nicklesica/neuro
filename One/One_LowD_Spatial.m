function One_LowD_Spatial(sound_1,date_1,pars)

%% Setup - edit this
sub_dir = 'All_1ms';
out_dir_name = 'Rearranged_Responses';
out_vars = {'IXA',strcat('SLD',pars.p_type)};
...
if exist(sprintf('R_%s',sound_1)),
    ...
    if length(IXA),
        ...
        load(sprintf('%s\\Nick\\Other\\Database_Analysis\\LowD_Basis_From_All_%s_%s\\c_c_x_x_x_62_1.mat',evalin('base','dropbox_dir'),pars.p_type,pars.r_name),...
            strcat(type,'_SFEAT_1'))
        
        FEAT = eval(strcat(type,'_SFEAT_1'));
        
        % Project onto basis
THIS ISNT FINISHED
        temp = reshape(R,n_cells,[]);
        temp = FEAT'*temp;
        ...
        eval(sprintf('SLD%s = temp;',pars.p_type));
