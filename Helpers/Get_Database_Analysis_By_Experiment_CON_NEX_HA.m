...
if length(date_list),
    ...
    eval(sprintf('[con_%s_%s] = Get_Database_Analysis_By_Experiment(date_list,''%s'',dir_str,var_str);',...
        var_str,end_str,var_name_str));
...
if length(date_list),
    ...
    eval(sprintf('[nex_%s_%s] = Get_Database_Analysis_By_Experiment(date_list,''%s'',dir_str,var_str);',...
        var_str,end_str,var_name_str));
    
    if ua_flag & length(strfind(var_name_str,'62')),
        
        eval(sprintf('[ua_%s_%s] = Get_Database_Analysis_By_Experiment(date_list,''%s'',dir_str,var_str);',...
            var_str,end_str,strrep(var_name_str,'62','83')));
...
%         eval(sprintf('ua_dates_%s_%s = [];',var_str,end_str));
    ...
    eval(sprintf('[ha_%s_%s] = Get_Database_Analysis_By_Experiment(date_list,''%s'',dir_str,var_str);',...
        var_str,end_str,var_name_str));
