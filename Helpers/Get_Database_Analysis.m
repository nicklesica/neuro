function [out,ix_valid,blocks,dates] = Get_Database_Analysis(date_list,var_name_str,dir_str,var_str)

two_sounds = (nargin>4);

% dbase_dir = strcat( evalin('base','dropbox_dir'),'\Nick\Other\Database_Analysis');
dbase_dir = sprintf('%s\\%s',evalin('base','database_analysis_dir'),dir_str);
...
% Figure out the total number of cells to be read
database_n_cells = evalin('base','database_n_cells');
database_dates = evalin('base','database_dates');
n_cells_all = sum(database_n_cells(ismember(database_dates,cellfun(@str2num, date_list))));
