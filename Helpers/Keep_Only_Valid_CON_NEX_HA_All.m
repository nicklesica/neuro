fprintf('\nKeeping only cells with valid data for all variables\n');
...
for i_type = 1:length(types),
    ...
    eval(sprintf('%s_ix = %s_ix_%s_%s;',this_type,this_type,var_strs{1},end_strs{1}));
    ...
    for i_var = 2:max([length(var_strs) length(end_strs)]),
        ...
        eval(sprintf('%s_ix = intersect(%s_ix,%s_ix_%s_%s);',this_type,this_type,this_type,var_str,end_str));
    end
end
...
for i_var = 1:max([length(var_strs) length(end_strs)]),
    ...
    for i_type = 1:length(types),
        ...
        if n_cells,
            ...
            eval(sprintf('%s_ix0_%s_%s = [];',this_type,var_str,end_str));
        end
        ...
        eval(sprintf('%s_ix_%s_%s = %s_ix;',this_type,var_str,end_str,this_type));
    ...
    for i_type = 1:length(types),
        ...
        eval(sprintf('fprintf(''%%d/%%d, '',nx,n_%s);',this_type));
